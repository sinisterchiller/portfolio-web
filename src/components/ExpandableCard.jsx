...
export function ExpandableCardDemo() {
  console.log('ExpandableCardDemo component rendering', {
  ...
  // Get base URL - try multiple methods to ensure it works in production
  const getBaseUrl = () => {
    // Try import.meta.env.BASE_URL first (Vite's way)
  ...
  // Generate cards with base URL applied at runtime, memoized for performance
  const cards = useMemo(() => {
    const processedCards = cardsData.map(card => ({
      ...card,
      src: baseUrl + card.src.replace(/^\//, ''),
    }));
    console.log('Cards generated:', processedCards.length, 'Base URL:', baseUrl, 'Sample src:', processedCards[0]?.src);
  ...
  useEffect(() => {
    if (active && typeof active === "object" && active.readmePath) {
      ...
      const readmePath = baseUrl + active.readmePath.replace(/^\//, '');
  ...
  console.log('Rendering ExpandableCardDemo with cards:', {
    ...
    <>
      ...
      {!cards || cards.length === 0 ? (
        <div className="max-w-6xl mx-auto w-full text-center py-8">
          <p className="text-neutral-400 mb-4">Loading projects...</p>
          <p className="text-neutral-500 text-sm">
            Cards: {cards ? cards.length : 'undefined'} | 
            Base URL: {baseUrl} | 
            CardsData: {cardsData ? cardsData.length : 'undefined'}
          </p>
        </div>
      ) : null}
